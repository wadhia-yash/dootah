...
for path in sorted(sys.argv[1:]):
    ...
    rows.append(row)
    for k in ("composables", "eligible", "remote", "mixed", "fallback", "ineligible", "files"):
        agg[k] += row[k]
...
print("| App | Compose funcs | OTA-capable | Mixed/native-boundary | Unsupported | Top blockers |")
print("|---|---|---|---|---|---|")
for r in rows:
    top = ", ".join(f"{k} {v}" for k, v in r["top"]) or "--"
    print(f"| {r['app']} | {r['composables']} | {r['remote']} | {r['mixed']} | "
          f"{r['fallback'] + r['ineligible']} | {top} |")
print()
print("TOTALS", dict(agg))
c = agg["composables"] or 1
e = agg["eligible"] or 1
print(f"\nof all composables: OTA-capable {100*agg['remote']/c:.1f}%  "
      f"mixed {100*agg['mixed']/c:.1f}%  fallback {100*agg['fallback']/c:.1f}%  "
      f"out-of-scope {100*agg['ineligible']/c:.1f}%")
print(f"of eligible only:   describable {100*(agg['remote']+agg['mixed'])/e:.1f}%")
