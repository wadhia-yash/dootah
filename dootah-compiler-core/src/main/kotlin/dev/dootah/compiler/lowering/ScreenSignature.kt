...
fun SourceNamedFunction.screenParameters(): List<ScreenParameter> =
    valueParameters.map { parameter -> parameter.classify() }

private fun SourceValueParameter.classify(): ScreenParameter {

    val name = this.name.asString()
    val type = returnTypeRef.coneTypeSafe<SourceType>()
 ...
 */
private fun SourceType.unitFunctionParameterTypes(): List<BundleType>? {
    ...
    val arguments = typeArguments.map { argument -> argument as? SourceType }
...
fun bundleTypeOf(type: SourceType?): BundleType? =
...
fun List<ScreenParameter>.valueParameters(): List<BundleParameter> =
...
fun List<ScreenParameter>.callbacks(): List<ScreenParameter.Callback> =
    filterIsInstance<ScreenParameter.Callback>()

fun List<ScreenParameter>.modifierName(): String? =
