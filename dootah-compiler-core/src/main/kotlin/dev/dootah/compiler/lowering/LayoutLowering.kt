 ...
 */
class LayoutLowering(
    private val rejector: Rejector,
    private val onAnchor: (String) -> Unit = {},
) {

    /** `Column(horizontalAlignment = Alignment.CenterHorizontally)`. */
    fun horizontalAlignment(expression: SourceExpression, layout: String): String? =
        alignment(expression, layout, "horizontalAlignment", Alignments.HORIZONTAL)

    /** `Row(verticalAlignment = Alignment.CenterVertically)`. */
    fun verticalAlignment(expression: SourceExpression, layout: String): String? =
        alignment(expression, layout, "verticalAlignment", Alignments.VERTICAL)

    /** `Box(contentAlignment = Alignment.Center)`. */
    fun boxAlignment(expression: SourceExpression, layout: String): String? =
        alignment(expression, layout, "contentAlignment", Alignments.BOX)

    /** `Column(verticalArrangement = Arrangement.spacedBy(8.dp))`. */
    fun verticalArrangement(expression: SourceExpression, layout: String): LayoutArrangement? =
        arrangement(expression, layout, "verticalArrangement", Arrangements.VERTICAL)

    /** `Row(horizontalArrangement = Arrangement.SpaceBetween)`. */
    fun horizontalArrangement(expression: SourceExpression, layout: String): LayoutArrangement? =
     ...
     */
    private fun memberName(expression: SourceExpression, owner: FqName): String? {
