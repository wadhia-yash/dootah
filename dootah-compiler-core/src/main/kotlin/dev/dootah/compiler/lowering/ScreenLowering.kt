 ...
 */
class ScreenLowering(
    private val function: SourceNamedFunction,
    ...
    private val functionName = function.qualifiedName
     ...
     * layout of its own.
     */
    private fun lowerBody(body: SourceBlock): BundleUi? {
        ...
        for (statement in body.statements) {
            ...
            when (val unwrapped = statement.unwrapReturn()) {
                ...
                is SourceProperty -> lowerDeclaration(unwrapped, into = prelude)
    ...
    private fun lowerDeclaration(property: SourceProperty, into: MutableList<BundleStatement>) {

        val name = property.name.asString()
        val type = bundleTypeOf(property.returnTypeRef.coneTypeSafe<SourceType>())
        ...
        if (type == null) {
            keepNative(
                ...
                detail = property.returnTypeRef.coneTypeSafe<SourceType>()
        ...
        if (value == null) {
            keepNative(
                ...
                code = RejectionCode.UNREADABLE_LOCAL_INITIALIZER,
                detail = (initializer as? SourceFunctionCall)
     ...
     */
    private fun SourceExpression.rememberedInitialValue(): SourceExpression? {

        val remember = this as? SourceFunctionCall ?: return null
        if (remember.resolvedCallableName() != REMEMBER) return null

        val produced = remember.arguments.lastOrNull()?.lambdaBody()
            ?.statements
            ?.firstNotNullOfOrNull { it.unwrapReturn() as? SourceFunctionCall }
            ?: return null
     ...
     */
    private fun lowerUiStatementPartially(statement: SourceElement): List<BundleUi>? {

        val direct = attempt { lowerUiStatement(statement) }
        if (!direct.failed) return direct.value

        val call = statement.unwrapReturn() as? SourceFunctionCall
        ...
        (statement.unwrapReturn() as? SourceWhenExpression)?.let { conditional ->
     ...
     */
    private fun noteWhatANativeRegionNeeded(element: SourceElement) {
     ...
     */
    private fun freeze(call: SourceFunctionCall): BundleUi? {
     ...
     */
    private fun freezeConditional(expression: SourceWhenExpression): BundleUi? {
    ...
    private fun SourceElement.drawsSomething(): Boolean {
        ...
        accept(
            object : SourceVisitor() {
                override fun visitElement(element: SourceElement) {
                    if (element is SourceFunctionCall && element.isComposableCall()) draws = true
    ...
    private fun SourceElement.sourceText(): String? {
    ...
    private fun lowerUiStatement(statement: SourceElement): List<BundleUi> =
        when (val unwrapped = statement.unwrapReturn()) {

            null -> emptyList()

            is SourceFunctionCall -> listOfNotNull(lowerUi(unwrapped))

            is SourceWhenExpression -> listOfNotNull(lowerConditionalUi(unwrapped))
    ...
    private fun lowerUi(call: SourceFunctionCall): BundleUi? =
    ...
    private fun lowerConditionalUi(expression: SourceWhenExpression): BundleUi? {
    ...
    private fun lowerText(call: SourceFunctionCall): BundleUi? {
     ...
     * the screen declares.
     */
    private fun lowerButton(call: SourceFunctionCall): BundleUi? {

        val mapping = call.resolvedArgumentMapping ?: return null

        var onClick: SourceExpression? = null
        var content: SourceBlock? = null
        var modifierList = emptyList<BundleModifier>()
     ...
     */
    private fun lowerClickHandler(expression: SourceExpression): List<BundleStatement>? {
        ...
        (expression as? SourcePropertyAccessExpression)?.resolvedName()?.let { name ->
     ...
     * cannot be built.
     */
    private fun lowerNativeComponent(call: SourceFunctionCall): BundleUi? {
     ...
     */
    private fun lowerBuilderEntries(builder: SourceAnonymousFunction): List<BundleEntry>? {
    ...
    private fun lowerBuilderStatement(statement: SourceElement): List<BundleEntry>? {

        val call = statement.unwrapReturn() as? SourceFunctionCall ?: return regionEntry(statement)
    ...
    private fun describedItem(call: SourceFunctionCall): BundleEntry? {

        val content = call.arguments
            .filterIsInstance<SourceAnonymousFunctionExpression>()
            .singleOrNull()
            ?.anonymousFunction
            ?: return null
     ...
     */
    private fun regionEntry(statement: SourceElement): List<BundleEntry>? {
     ...
     */
    private fun lowerContent(content: SourceAnonymousFunction): List<BundleUi>? {
     ...
     */
    private fun SourceElement.readsAnOuterReceiver(): Boolean {

        val introducedHere = mutableSetOf<SourceSymbol>()

        accept(
            object : SourceVisitor() {
                override fun visitElement(element: SourceElement) {
                    if (element is SourceAnonymousFunction) introducedHere += element.symbol
                    element.acceptChildren(this)
                }
            }
        )
        ...
        accept(
            object : SourceVisitor() {
                override fun visitElement(element: SourceElement) {
                    if (element is SourceThisReceiverExpression &&
     ...
     */
    private fun SourceThisReceiverExpression.introducedIn(
        introduced: Set<SourceSymbol>,
    ): Boolean {
    ...
    private fun SourceElement.declarationsReadFromBody(): List<String> {
        ...
        accept(
            object : SourceVisitor() {
                ...
                override fun visitElement(element: SourceElement) {
                    if (element is SourcePropertyAccessExpression) {
    ...
    private fun lowerStatements(statements: List<SourceElement>): List<BundleStatement>? {
        ...
        for (statement in statements) {
            ...
            when (val unwrapped = statement.unwrapReturn()) {
                ...
                is SourceProperty -> lowerDeclaration(unwrapped, into = lowered)

                is SourceVariableAssignment -> lowered += lowerAssignment(unwrapped) ?: return null

                is SourceWhenExpression -> lowered += lowerConditionalStatement(unwrapped)
                    ?: return null

                is SourceFunctionCall -> lowered += lowerCallStatement(unwrapped) ?: return null
    ...
    private fun lowerAssignment(assignment: SourceVariableAssignment): BundleStatement? {

        val target = (assignment.lValue as? SourcePropertyAccessExpression)?.resolvedName()
    ...
    private fun lowerCallStatement(call: SourceFunctionCall): BundleStatement? {
    ...
    private fun SourceFunctionCall.invokedCallbackName(): String? {

        val receiverName = (explicitReceiver as? SourcePropertyAccessExpression)?.resolvedName()
    ...
    private fun lowerConditionalStatement(expression: SourceWhenExpression): List<BundleStatement>? {
    ...
    private fun <T> lowerBranches(
        ...
        lowerBody: (SourceElement) -> List<T>,
    ): List<Pair<BundleExpression, List<T>>>? {

        val subject = expression.subjectName

        if (subject != null) {
            val initializer = expression.subjectInitializer ?: return null
            val lowered = lowerExpression(initializer) ?: return null
            subjects += subject to lowered
        }
    ...
    private fun lowerExpression(expression: SourceExpression): BundleExpression? =
        when (expression) {

            is SourceLiteralExpression -> lowerLiteral(expression)
            is SourceWhenSubjectExpression -> subjects.lastOrNull { it.first == expression.subjectName }?.second

            is SourceStringConcatenationCall -> lowerInterpolation(expression)

            is SourceEqualityOperatorCall -> lowerEquality(expression)

            is SourceComparisonExpression -> lowerComparison(expression)

            is SourceBooleanOperatorExpression -> lowerBooleanOperator(expression)

            is SourceWhenExpression -> lowerConditionalExpression(expression)

            is SourcePropertyAccessExpression -> lowerReference(expression)

            is SourceFunctionCall -> lowerCallExpression(expression)
     ...
     */
    private fun lowerLiteral(literal: SourceLiteralExpression): BundleExpression? {
    ...
    private fun lowerReference(access: SourcePropertyAccessExpression): BundleExpression? {
        ...
        // the wrong thing.
        val receiver = (access.explicitReceiver as? SourcePropertyAccessExpression)?.resolvedName()
    ...
    private fun lowerInterpolation(call: SourceStringConcatenationCall): BundleExpression? {
    ...
    private fun lowerEquality(call: SourceEqualityOperatorCall): BundleExpression? {
    ...
    private fun lowerComparison(comparison: SourceComparisonExpression): BundleExpression? {
    ...
    private fun lowerBooleanOperator(
        expression: SourceBooleanOperatorExpression,
    ): BundleExpression? {
    ...
    private fun lowerConditionalExpression(expression: SourceWhenExpression): BundleExpression? {

        val branches = lowerBranches(expression) { branch ->
            listOfNotNull((branch.unwrapReturn() as? SourceExpression)?.let { lowerExpression(it) })
        } ?: return null
    ...
    private fun lowerCallExpression(call: SourceFunctionCall): BundleExpression? {
     ...
     */
    private fun lowerBundledCall(call: SourceFunctionCall): BundleExpression? {

        val symbol = call.calleeReference.toResolvedCallableSymbol()
        val declaration = symbol?.declaration as? SourceNamedFunction
    ...
    private fun lowerFunction(name: String, declaration: SourceNamedFunction): Boolean {

        val returnType = bundleTypeOf(declaration.returnTypeRef.coneTypeSafe<SourceType>())
        ...
        val expression = body.statements
            .mapNotNull { it.unwrapReturn() as? SourceExpression }
            .singleOrNull()
    ...
    private fun SourceFunctionCall.isComposableCall(): Boolean = isComposable()

    private fun SourceExpression.lambdaBody(): SourceBlock? =
        (this as? SourceAnonymousFunctionExpression)?.anonymousFunction?.body
 ...
 */
private fun SourceBlock.prologueDeclarations(): Set<String> {

    val declared = statements
        .filterIsInstance<SourceProperty>()
    ...
    for (statement in statements) {
        ...
        if (statement is SourceProperty) {
 ...
 */
private fun SourceElement.assignsOneOf(names: Set<String>): Boolean {
    ...
    accept(
        object : SourceVisitor() {
            ...
            override fun visitElement(element: SourceElement) {
                ...
                if (element is SourceAnonymousFunctionExpression && !element.isComposableContent()) {
                    return
                }

                if (element is SourceVariableAssignment) {
                    val target = (element.lValue as? SourcePropertyAccessExpression)?.resolvedName()
...
private fun SourceAnonymousFunctionExpression.isComposableContent(): Boolean =
    anonymousFunction.typeRef.coneTypeSafe<SourceType>()
        ?.isComposableFunctionType() == true

private fun SourceFunctionCall.isComposable(): Boolean =
 ...
 */
fun SourceElement.unwrapReturn(): SourceElement? {
    ...
    if (this is SourceUnitExpression) return null

    if (this !is SourceReturnExpression) return this

    return when (val produced = result) {
        is SourceUnitExpression -> null
        is SourceFunctionCall, is SourceWhenExpression -> produced
        is SourceLiteralExpression -> if (produced.value == null) null else produced
        else -> if (produced.source == null) null else produced
    }
}

fun SourceElement.sourceOffset(): Int? = source?.startOffset
 ...
 */
private fun SourceExpression.isElse(): Boolean = this is SourceElseIfTrueCondition
