...
const val GENERATED_PACKAGE = "dev.dootah.generated"
...
object BundleSourceWriter {
    ...
    fun writeScreen(screen: BundleScreen): String = buildString {
        ...
        appendLine()
        appendLine("import protocol.CallbackArgument")
        ...
        appendLine()
        appendLine("object ${screenObjectName(screen)} {")
        appendLine()
        ...
        is BundleCommandModel.InvokeCallback ->
            "Command.InvokeCallback(${kotlinStringLiteral(command.name)}" +
                if (command.arguments.isEmpty()) ")"
                    ...
                    command.arguments.joinToString(", ") { renderCallbackArgument(it) } +
                    "))"
    ...
    /**
     * Writes one callback value in the shape the wire carries.
     *
     * The conversion is chosen by the parameter's declared type rather than by
     * what the expression looks like, so the value the app coerces back is the
     * one its own signature asked for. `Long` goes as text; every other number
     * goes as a double, which is what a JavaScript number already is.
     */
    private fun renderCallbackArgument(argument: BundleCallbackArgument): String {

        val value = renderExpression(argument.value)

        return when (argument.type) {
            BundleType.STRING -> "CallbackArgument.Text($value)"
            BundleType.LONG -> "CallbackArgument.Text(($value).toString())"
            BundleType.BOOLEAN -> "CallbackArgument.Bool($value)"
            BundleType.INT,
            BundleType.FLOAT,
            BundleType.DOUBLE -> "CallbackArgument.Number(($value).toDouble())"
        }
    }
