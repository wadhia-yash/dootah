...
public class PublicationStore {
    ...
    static void require(boolean valid) { if (!valid) throw new IllegalArgumentException("Invalid publication"); }
    static String digest(byte[] bytes) throws Exception {
        return HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes));
    }
    private Path artifact(String hash) {
        require(hash.matches("[0-9a-f]{64}")); return artifacts.resolve(hash);
    }
    public synchronized void upload(String hash, InputStream input) throws Exception {
        ...
        require(bytes.length > 0 && bytes.length <= MAX_ARTIFACT && digest(bytes).equals(hash));
        ...
        try (var c = FileChannel.open(artifacts.resolve(".lock"), StandardOpenOption.CREATE, StandardOpenOption.WRITE);
            ...
            if (Files.exists(target)) {
                if (!Arrays.equals(Files.readAllBytes(target), bytes)) throw new Conflict();
                return;
            }
    ...
    public byte[] download(String hash) throws Exception {
        byte[] bytes = Files.readAllBytes(artifact(hash));
        require(bytes.length <= MAX_ARTIFACT && digest(bytes).equals(hash)); return bytes;
    }
            ...
            for (var old : existing) {
                if (old.identity().equals(release.identity())) {
                    if (!old.channel().equals(release.channel()) || !old.manifest().equals(release.manifest())
                            ...
                            || !Objects.equals(old.maxAppVersion(), release.maxAppVersion())) throw new Conflict();
                ...
                if (old.appId().equals(release.appId()) && old.runtimeVersion().equals(release.runtimeVersion())
                        && old.channel().equals(release.channel()) && old.version() == release.version()) throw new Conflict();
            }
    ...
    public synchronized Map<String, String> rollout(String identity, JsonNode control) throws Exception {
        require(identity.matches("[0-9a-f]{64}"));
        require(control.isObject() && control.size() == 2 && control.has("rolloutPercent") && control.has("paused"));
        return mutate(root -> {
            ...
            throw new IllegalArgumentException("Unknown release");
        });
    }
