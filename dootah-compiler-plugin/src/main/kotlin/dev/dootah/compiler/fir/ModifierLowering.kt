...
internal class ModifierLowering(
    ...
    fun lower(expression: FirExpression): List<BundleModifier>? {
    ...
    private fun lowerEntry(call: FirFunctionCall): BundleModifier? {
     ...
     */
    private fun colorOf(expression: FirExpression): Long? {
        ...
        val packed = call
            ...
            ?.arguments
            ?.singleOrNull()
            ?.let { (it as? FirLiteralExpression)?.value }
    ...
    private fun FirFunctionCall.fraction(): Double? {
    ...
    private fun FirFunctionCall.weight(): Double? {
...
internal fun FirFunctionCall.resolvedCallableName(): FqName? =
    calleeReference.toResolvedCallableSymbol()?.callableId?.asSingleFqName()

internal fun FirPropertyAccessExpression.resolvedCallableName(): FqName? =
    calleeReference.toResolvedCallableSymbol()?.callableId?.asSingleFqName()
 ...
 * screen reads.
 */
internal fun FirPropertyAccessExpression.resolvedName(): String? =
    (calleeReference as? FirResolvedNamedReference)?.name?.asString()
